...
const getModuleInfo = (pathToModule) => {
...
const copyModuleBuild = async (pathToModule, moduleName) => {
...
const deployModule = async () => {
  ...
  const moduleInfo = skipBuild ? getModuleInfo(modulePath) : await buildModule(modulePath);
  const moduleLocation = await copyModuleBuild(modulePath, moduleInfo.moduleName);
